...
class EventRepository {
  ...
  Future<List<EventModel>> searchEvents({
    ...
    // Make query India-specific if not already
  ...
  Future<List<EventModel>> _callApi(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print('🎉 ${items.length} events returned');
  ...
  EventModel? _parse(Map<String, dynamic> item, int index) {
    try {
      ...
      if (v != null) {
        ...
        // Coords
        final rawLat = v['latitude'];
        final rawLng = v['longitude'];
        final pLat = double.tryParse(rawLat?.toString() ?? '');
        final pLng = double.tryParse(rawLng?.toString() ?? '');

        if (pLat != null && pLng != null && pLat != 0 && pLng != 0) {
          eLat = pLat;
          eLng = pLng;
        } else {
          // Fall back to known city coords
  ...
  String _category(String text) {
    ...
    if (t.contains('concert') || t.contains('music') || t.contains('dj') ||
        t.contains('band') || t.contains('singer') || t.contains('garba') ||
        t.contains('bollywood') || t.contains('sufi')) return 'music';
    if (t.contains('cricket') || t.contains('ipl') ||
        t.contains('football') || t.contains('match') ||
        t.contains('sport') || t.contains('kabaddi') ||
        t.contains('badminton') || t.contains('tennis') ||
        t.contains('marathon')) return 'sports';
    if (t.contains('comedy') || t.contains('standup') ||
        t.contains('theatre') || t.contains('dance') ||
        t.contains('art') || t.contains('exhibition') ||
        t.contains('film') || t.contains('movie')) return 'art';
    if (t.contains('tech') || t.contains('workshop') ||
        t.contains('hackathon') || t.contains('startup') ||
        t.contains('meetup') || t.contains('conference') ||
        t.contains('summit') || t.contains('flutter') ||
        t.contains('ai ') || t.contains(' ml ')) return 'tech';
    if (t.contains('food') || t.contains('mela') ||
        t.contains('fair') || t.contains('carnival') ||
        t.contains('cuisine') || t.contains('culinary')) return 'food';
    if (t.contains('yoga') || t.contains('wellness') ||
        t.contains('fitness') || t.contains('meditation') ||
        t.contains('community') || t.contains('charity') ||
        t.contains('spiritual')) return 'community';
    return 'music';
  }
}