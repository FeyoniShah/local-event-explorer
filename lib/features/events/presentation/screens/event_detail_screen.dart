...
class _EventDetailScreenState extends ConsumerState<EventDetailScreen> {
  ...
  // ── FIX: find event AFTER first frame, never inside build ──────────────
  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) => _findEvent());
  }

  void _findEvent() {
    final eventsAsync = ref.read(eventsProvider);
    final searchAsync = ref.read(searchEventsProvider);
  ...
  void _goToMap(EventModel event) => context.go('/map');
  ...
  Widget build(BuildContext context) {
    // ── NO whenData calls here anymore ─────────────────────────────────
    if (_event == null) {
        ...
        slivers: [
          SliverAppBar(
            ...
            leading: IconButton(
              ...
              onPressed: _handleBack, // ← fixes back button
            ),
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: [
                      ...
                      Chip(
                        label: Text('📍 ${event.city}', style: const TextStyle(fontSize: 12)),
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          ...
                          const Divider(height: 16),
                          _infoRow(Icons.confirmation_number, 'Price',
                              event.isFree
                                  ? '🎉 Free Entry'
                                  : event.price != null
                  ...
                  // Book Tickets
                  if (event.externalUrl != null && event.externalUrl!.isNotEmpty)
                    SizedBox(
                      width: double.infinity,
                  ...
                  if (!eventPassed)
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        onPressed: () async {
                          await ref.read(rsvpProvider.notifier).toggleRsvp(event);
                          if (mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                              content: Text(isRsvpd
                                  ? 'RSVP removed'
                                  : '🎉 You\'re going! Check your profile.'),
                              backgroundColor: isRsvpd ? Colors.grey[700] : Colors.green[700],
                              duration: const Duration(seconds: 2),
                            ));
                          }
                        },
                        icon: Icon(isRsvpd ? Icons.check_circle : Icons.event_available),
                        label: Text(isRsvpd ? "I'm Going ✓ (tap to undo)" : "RSVP — I'm Going!"),
                        style: ElevatedButton.styleFrom(
                          backgroundColor: isRsvpd ? Colors.green[700] : Colors.green,
  ...
  String _getCategoryEmoji(String c) {
    switch (c) {
      case 'music': return '🎵';
      case 'sports': return '🏏';
      case 'art': return '🎭';
